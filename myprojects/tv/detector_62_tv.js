...
(function () {
    ...
    let received_gameId = false;
    function Create_Game(base64Image){

        received_gameId = true;
        


        }

    function Create_New_Game(base64Image){
        countDown_count = 0;
        received_gameId = true;
        Restart_The_Game();
        ...
        axios.post('https://fifa1.hashconnect.in/api/user/game', {
          ...
          .then(function (response) {
            ...
            gameDurationTimer = response.data.data.config.gameDurationTimer;
        ...
        console.log(" Record a Shot is called ============================================" + "gameId " +gameId);

         axios.post('https://fifa1.hashconnect.in/api/user/game/'+gameId+'/shot/'+shotId, {
    ...
    function show_end_screen(){
        ...
        function Submit_Timeout(){
            ...
            console.log(" Submit_Timeout is called *************************************************");
            ...
            axios.post('https://fifa1.hashconnect.in/api/user/game/'+gameId+'/submit', {
              ...
              .then(function (response) {
                ...
                //console.log(" Submit_Timeout response ****************");
                //console.log(response);
...
function Quit_Game(){
    ...
    stopCamera(video); 
    clearGroup(scene);
    //cancelAnimationFrame(main_anim_Id);
}
...
function stopCamera(videoElement) {
  ...
  if (stream) {
    stream.getTracks().forEach(track => {
      track.stop(); // 🔥 THIS actually turns off camera hardware
    });

    videoElement.srcObject = null; // detach stream
  }
}
//
...
function removeModel(model) {
  if (!model) return;
    console.log("removeModel called");
...
let gameId = null;
let baseScore = 100;
let cornerMultiplier = 1.5;
let gameDurationTimer = 180+180;
let shotId = 0
let isGoal = false;
let position = "net";
let timer_mili = 0;
let current_kick_score = 0;
    // === DOM CREATION ===
...
document.body.style.fontFamily = 'sans-serif';
    ...
    //bufferCanvas.style.zIndex = '11';
    //bufferCanvas.width = dbgCanvas.width;
    //bufferCanvas.height = dbgCanvas.height;
...
let dummy_video_grab = null;
    ...
    function checkStability(rect) {
        ...
        if (stabilityCounter >= LOCK_TARGET) {//LOCK_TARGET
            ...
            // Convert canvas to base64
             //base64Image = dbgCanvas.toDataURL("image/png"); 
              base64Image = bufferCanvas.toDataURL("image/png"); 
    ...
    function blink_viewfinder(){
        ...
        if(pulse_snd_flag){// play pulse sound in loop
            play_target_found_Sound();
        ...
        if (viewfinder_flash_count > 15) {
          ...
          if(total_viewfinder_flash_count > 10){//10
            ...
            console.log("blink_viewfinder finish");
                        //dbgCtx.clearRect(0, 0, dbgCanvas.width, dbgCanvas.height);
            //dbgCtx.drawImage(dummy_video_grab, 0, 0, dbgCanvas.width, dbgCanvas.height);
    ...
    function disposeBuffer() {
        if (bufferCanvas) {
            ...
            // Force VRAM release by resizing to 0
...
link_1.href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400..900&display=swap";
...
scene = new THREE.Scene();
        ...
        //gw show gameworld
          ...
          gameWorld.position.set(0 ,0.5, 2.3);//gameWorld.position.set(0 ,1, =2.0) // original
          //gameWorld.position.set(0 ,0.0, 2.3);
          ...
          camera.position.set(0, 3.7, (camZ*-1) + screen_ratio);
          ...
          camera.lookAt(0, 1.9-screen_ratio*2, 20);
...
//scannerOvl.style.display = 'none';
  ...
  //animate();
    ...
    //const brandingTexture = textureLoader.load("./branding_boundry_1.png");
    //const brandingTexture = textureLoader.load("./branding_boundry_2_tv.png");
    const brandingTexture = textureLoader.load("./branding_boundry_3_tv.png");//no branding
    brandingTexture.colorSpace = THREE.SRGBColorSpace;
    brandingTexture.anisotropy = renderer.capabilities.getMaxAnisotropy();
    brandingTexture.wrapS = THREE.RepeatWrapping;
    brandingTexture.repeat.set(6, 1); // repeat branding

    const boardWidth = 20;
    const boardHeight = 0.8;

    const brandingGeometry = new THREE.PlaneGeometry(boardWidth, boardHeight);
    ...
    const brandingBoard = new THREE.Mesh(brandingGeometry, brandingMaterial);
    //brandingBoard.position.set(0, 1, 11.5); 
    brandingBoard.position.set(0, 0.4, 12.55); 
    brandingBoard.rotation.y = 91;
    //brandingBoard.rotation.x = -0.08;
    
    //brandingTexture.colorSpace = THREE.SRGBColorSpace;
    brandingTexture.wrapS = THREE.RepeatWrapping;
    brandingTexture.wrapT = THREE.ClampToEdgeWrapping; // vertical no repeat
    brandingTexture.repeat.set(8.7, 1); // 10 tiles horizontally
   //brandingBoard.position.y = 0;
  //brandingBoard.position.z = 5;
    gameWorld.add(brandingBoard);
...
function formatTime(ms) {
  ...
  timer_mili = ms;
  //console.log(timer_mili + "==" + gameDurationTimer);
  if(timer_mili >= gameDurationTimer*1000){
...
var animRanges = {//new
  ...
  //punt: [122, 234],
  //roll: [235, 359],
  save_lower_center: [79, 112],
  save_lower_left: [174, 243],//[174, 269],//save_lower_left: [174, 290],
  save_lower_right: [291, 361], //[291, 407],
  save_upper_center: [27, 62],
  save_upper_left: [408, 477],//[408, 549],
  save_upper_right: [550, 603],//[550, 691],
  //throw_ball: [998, 1174]
};
...
function shootBall() {
  ...
  if (ballInFlight && currentAttempt < total_kick_count) return;
...
function updateBallPosition(delta) {
 ...
 camera.position.x += (ball.position.x*1.5 - camera.position.x)/cam_sp;
 camera.position.y += (camY - camera.position.y)/cam_sp;
 camera.position.z += ((-camZ+screen_ratio) - camera.position.z)/cam_sp;
      ...
      ballInFlight = false;
        ...
        //velocity.y=0;
          fadeBall();
            ...
            // if this condition is commented then if ball goes above the goalpost it shows the goal....this condition handle that 
            //hit_Flag = true;
            if(list_id != 0){
              list_id = 3;
              position = "missed";
             current_kick_score = 0;
             isGoal = false;
...
function handlePostCollision(post) {
  ...
  switch (post.name) {
    case 'leftPost':
      isGoal_done = isBallOnGoalSideOfPost(post);

      if(isGoal_done){
        list_id = 2;
        playKick_Sound();
        score_multiplyer = cornerMultiplier;//1.5;
        current_kick_score = baseScore * score_multiplyer;
        position = "left pole";
      ...
      //velocity.x += isGoal_done ? 0.5 : deflectionStrength;
      break;
    ...
    case 'net':
     ...
     velocity.z *= 0.00002;
     isGoal_done = true; 
     score_multiplyer = cornerMultiplier/cornerMultiplier;//1;
     current_kick_score = baseScore * score_multiplyer;
     position = "net";
     list_id = 1;
  ...
  //console.log(`Post hit: ${post.name} → Goal side? ${isGoal_done ? '✅ YES' : '❌ NO'}`);
  //console.log("isGoal_done = "+isGoal_done);
  fadeBall();
  if(isGoal_done){
    isGoal_done =false;
    //ball.visible = false;
    ...
    flashScore();
    //createScorePopup();
    const text =  baseScore * score_multiplyer;
    animateRating(text);
    ...
    if(list_id != 0){
        list_id = 3; // missed the ball completely
    }
    //showGoalFlash();
...
countDown_Text.style.color = '#FFFFFF'; // kelloggs red core text
...
countDown_Text.style.textShadow = `
  0 0 5px #000000,    /* soft black halo */
  0 0 10px #000000
...
countDown_Text.style.fontSize = '200px';
countDown_Text.style.fontWeight = '400';//ff
countDown_Text.style.zIndex = 2011
...
const countDown_copy_list = ["3", "2", "1"];
...
countDown_Text.style.display = 'block'
 ...
 // Start big and transparent
  ...
  countDown_Text.style.opacity = '1';
    ...
    countDown_count++;
    //console.log("countDown_count ="+ countDown_count , countDown_copy_list.length);
...
goalFlash.style.fontFamily = "Noto Sans", 'system-ui';
...
//goalFlash.style.maxWidth = '90vw';  
goalFlash.style.textAlign = 'center';    // optional — center text nicely
goalFlash.style.whiteSpace = "pre-line";
...
const copy_colors = {
  0: '#ffffff',  // orange - Goal Top Bar
  1: '#ffffff',  // red - Goal Net
  2: '#ffffff',  // white - Goal Side Bar
  3: '#ffffff',  // orange - missed
  4: '#ffffff'   // red - saved by goli
  
};
const textShadows = {
  0: '0 0 5px #000000, 0 0 10px #000000',
  1: '0 0 5px #000000, 0 0 10px #000000',
  2: '0 0 5px #000000, 0 0 10px #000000',
  3: '0 0 5px #000000, 0 0 10px #000000',
  4: '0 0 5px #000000, 0 0 10px #000000'
};
...
//ys
 const scan_status_msg = document.getElementById('scan_status_msg');



 scan_status_msg.innerHTML = "Scan Your TV to Play";
 //scan_status_msg.style.fontSize = 'clamp(2rem, 3vw, 1rem)';
 /*scan_status_msg.innerHTML = `<svg viewBox="0 0 24 24">
 ...
 scan_status_msg.style.textShadow = '0 0 5px #000000';
  ...
  if(showGoalFlash_msg) return;
   ...
   goalFlash.style.top = '55%';
    ...
    goalFlash.style.opacity = '1';
     ...
     if (list_id == 4){
       fadeBall();///hit the goli
     }



     console.log("current_kick_score = " + current_kick_score);
      ...
      //textNode.textContent = eval("copy_list_"+list_id);//[randomCount];
        if(list_id <= 2){
            ...
            //current_kick_score = baseScore * score_multiplyer;
            textNode.textContent = eval("copy_list_"+list_id)+ "+" + current_kick_score;
            
         }else{
            registerShot(false);//no goal
            //current_kick_score = 0;
            textNode.textContent = eval("copy_list_"+list_id);
         }
      ...
      goalFlash.style.transform = 'translate(-50%, -50%) scale(1.2)';
      goalFlash.style.opacity = '1';
...
function loading_check(){
    ...
    if(!check)return
    if(model_loaded_count >= model_total_count && sound_loaded_count >= sound_total_count){
        check = false;
        ...
        //----
        console.log("loading_check is finished" + model_loaded_count);

        cancelAnimationFrame(loading_check_id);
    }
    loading_check_id = requestAnimationFrame(loading_check);
}
//----============---------
...
function registerShot(isGoal_done) {
  ...
  if (currentAttempt <= total_kick_count) {//return;
...
masterGain.gain.value = 1; // global volume
...
kickGain.gain.value = 0.5;   // medium kick sound
...
function play_target_found_Sound() {
  ...
  /*source.onended = () => {
    source.disconnect(); // remove from audio graph
  };*/
}
...
//goal
  ...
  function playGoal_Sound() {
  const source = audioContext.createBufferSource();
  source.buffer = goalBuffer;
  source.connect(goalGain);
  source.start(0);
...
//whistle
  ...
  .then(buffer => {
    sound_loaded_count++;
    whistle_Buffer = buffer;
  });

  function play_Whistle_Sound() {
  const source = audioContext.createBufferSource();
  source.buffer = whistle_Buffer;
  source.connect(whistleGain);
  source.start(0);
...
//boo
  ...
  const source = audioContext.createBufferSource();
  source.buffer = boo_Buffer;
  source.connect(booGain);
  source.start(0);
...
//countdown_trumpet
  ...
  source.connect(countdown_trumpetGain);
  source.start(0);
...
function showGoal(){
  const el = document.getElementById("scan_status_msg");

  el.innerHTML = `
  <svg viewBox="0 0 24 24">
    <circle cx="12" cy="12" r="13" fill="none"/>
    <path d="M7 12l3 3 7-7" stroke="white" stroke-width="2.1"
 ...
 */
 let total_kick_count = 50;
 let testing_is_on = false;
 if(testing_is_on){
    total_kick_count = 50;
 }else{
    total_kick_count = 5;
...
hideScorePanel();
timer_wrapper.style.display = 'none';
const btn1 = document.querySelector(".btn1");// popup icons reset close help
btn1.style.display = 'none';
