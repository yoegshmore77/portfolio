...
document.body.style.fontFamily = 'sans-serif';
    ...
    function processVideo() {
        ...
        const roiH = Math.floor(roiW * 7 / 10);  // 10:7 aspect ratio
    ...
    function finalize(cvBest, aiPreds, vw, vh) {
        ...
        if (!pick && aiPreds.length) {
            ...
            const _roiW = Math.floor(vw * 0.90);
            const _roiH = Math.floor(_roiW * 7 / 10);
    ...
    function animate_game_container(){
            ...
            gameWorld.scale.lerp(sc, 0.03); // micro smoothing
...
function makeSubclip(baseClip, name, start, end) {
  ...
  var subClip = THREE.AnimationUtils.subclip(baseClip, name, start, end);
...
function playNextIdle() {
  ...
  // ----------------------------------------
      ...
      var subClip = makeSubclip(clip, name, start, end);
...
function playSegment(name) {
  ...
  var subClip = makeSubclip(clip, name, range[0], range[1]);
  action = keeperMixer.clipAction(subClip);
  action.reset();
  action.setLoop(THREE.LoopOnce);
  action.clampWhenFinished = true;
...
let gameWorld;
        ...
        //gw show gameworld
          ...
          gameWorld.position.set(0,1,-1.1)
          gameWorld.scale.set(2.5,2.5,2.5);
          ...
          camera.lookAt(0, 1.5, 25);      //for style_4.css avweage viewfinder aspect-ratio: 10 / 6;
...
function init_1(){
    ...
    goalkeeper.traverse((obj) => {
      ...
      if (obj.isSkinnedMesh) {
        //console.log("Found a skinned mesh:", obj.name);

        // Skeleton + bones
        //console.log("Skeleton bones:", obj.skeleton.bones);

        // Example: list bone names
        obj.skeleton.bones.forEach((bone, i) => {
          //console.log("bone "+ i, bone.name);

          if(bone.name == "Bip001_Head"){//mixamorigHead_06
            //console.log("head bone found");
            //console.log(bone.position);
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }
          
          if(bone.name == "Bip001_L_Clavicle"){//collar bone
            hitSpheres.push(createHitSphere(sp_size_1, bone));
          }
          if(bone.name == "Bip001_R_Clavicle"){//collar bone
            hitSpheres.push(createHitSphere(sp_size_1, bone));
          }

          if(bone.name == "Bip001_Spine"){
            //hitSpheres.push(createHitSphere(5, bone));
          }

          if(bone.name == "Bip001_Pelvis"){//waist
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }

          if(bone.name == "Bip001_L_Hand"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }
          if(bone.name == "Bip001_R_Hand"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }

          if(bone.name == "Bip001_L_UpperArm"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }
          if(bone.name == "Bip001_R_UpperArm"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }

          if(bone.name == "Bip001_L_Forearm"){
            hitSpheres.push(createHitSphere(sp_size_1, bone));
          }
          if(bone.name == "Bip001_R_Forearm"){
            hitSpheres.push(createHitSphere(sp_size_1, bone));
          }

          if(bone.name == "Bip001_L_Thigh"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }
          if(bone.name == "Bip001_R_Thigh"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }

          if(bone.name == "Bip001_L_Calf"){
            hitSpheres.push(createHitSphere(sp_size_1, bone));
          }
          if(bone.name == "Bip001_R_Calf"){
            hitSpheres.push(createHitSphere(sp_size_1, bone));
          }

          if(bone.name == "Bip001_L_Foot"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }
          if(bone.name == "Bip001_R_Foot"){
            hitSpheres.push(createHitSphere(sp_size_2, bone));
          }
...
var animRanges = {
  ...
  punt: [122, 234],
  roll: [235, 359],
  save_lower_center: [360, 432],
  save_lower_left: [433, 553],
  save_lower_right: [554, 674],
  save_upper_center: [675, 745],
  save_upper_left: [746, 871],
  save_upper_right: [872, 997],
  throw_ball: [998, 1174]
};
...
/*
  ...
  //const timeStep = 0.05;//0.05;
  //const totalTime = 0.1;//1;//1.5;//arc distance

  const timeStep = 0.05;//0.05;
  const totalTime = 1;//1;//1.5;//arc distance
  ...
  const sphereGeometry = new THREE.SphereGeometry(0.04, 12, 12); // smooth small spheres
  //const stepInterval = 2;//1; // spacing between spheres higher number increases the spacing
  const stepInterval = 1;//1; // spacing between spheres
  let stepCount = 0;
  ...
  */
  ...
  const velocity = getInitialVelocity(currentAngle, currentForce*0.5).clone();
  let pos = startPos.clone();

  // ⚙️ Base geometry and material
  const sphereGeometry = new THREE.SphereGeometry(0.04, 12, 12); // smooth small spheres
  //const stepInterval = 2;//1; // spacing between spheres higher number increases the spacing
  const stepInterval = 0.5;//1; // spacing between spheres
  let stepCount = 0;

  for (let t = 0; t < totalTime; t += timeStep) {
    applyLateralAcceleration(velocity, currentAngle*0.5, currentSwing*0.5, timeStep);
    applyGravity(velocity, timeStep);
    pos.addScaledVector(velocity, timeStep);
    ...
    if (stepCount % stepInterval === 0) {
      ...
      const fade = t / totalTime;
      const opacity = THREE.MathUtils.lerp(1.0, 0.1, fade);
      const scale = THREE.MathUtils.lerp(1.0, 0.9, fade);
...
function spawnConfettiBurst(worldPos, count = 50) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    sprite.scale.setScalar(0.15 + Math.random() * 0.35); // particle size
...
function shakeCamera(intensity = 0.09, duration = 100) {
  ...
  setTimeout(() => {
    ...
    const speedSlow   = 0.15; // 25% speed for slow motion
