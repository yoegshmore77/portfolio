...
(function () {
    ...
    function processVideo() {
        ...
        // ROI box (center 60%)
        const roiX = Math.floor(vw * 0.20), roiY = Math.floor(vh * 0.20);
        const roiW = Math.floor(vw * 0.60), roiH = Math.floor(vh * 0.60);
    ...
    function runCV(vw, vh, roiX, roiY, roiW, roiH) {
        try {
            ...
            const minA = roiW * roiH * 0.02;  // 2% of ROI
            ...
            for (let i = 0; i < contours.size(); i++) {
                ...
                cv.approxPolyDP(cnt, approx, 0.04 * peri, true);  // Coarser = more lenient on curves
                const v = approx.rows;
                approx.delete();
                ...
                // SIMPLE CHECK:
                //   3-8 vertices, fill > 0.3, aspect 0.3-4.0
                const ok = v >= 3 && v <= 15 && fill > 0.2 && aspect >= 0.3 && aspect <= 4.0;
    ...
    function finalize(cvBest, aiPreds, vw, vh) {
        ...
        let pick = cvBest;  // CV is the ONLY source

        // If AI found something overlapping, use its label
