//derived from index_61_only_game
//no backend
...
let ball, goalkeeper, goalpost, ground, pringles_tin;
let currentForce = 0;
let currentAngle = 0;
...
function init_1(){
    ...
    //scene.add(goalpost);
    //gameWorld.add(pringles_tin);
    //pringles_tin.position.set(0, 0, 11);
    // maybe scale if needed
    //pringles_tin.scale.set(1.5, 1.5, 1.5);
    
    //pringles_tin.rotation.set(0, 110, 0);

    pringles_tin.traverse(child => {
      if (child.isMesh && child.material) {
          const mats = Array.isArray(child.material) ? child.material : [child.material];
          mats.forEach(mat => {
    ...
    console.error('Error loading tin model:', error);
  });
    ...
    goalkeeper.traverse((obj) => {
        ...
        if (obj.isBone) {
            ...
            if(obj.name == "Bone_Brow_C"){
                ...
                pringles_tin.rotation.set(1.7,109.9,89.5);

            }
            if(obj.name == "neck_01"){
                hitSpheres.push(createHitSphere(10, obj));

                obj.add(pringles_tin); // attach to bone
...
//scannerOvl.style.display = 'none';
  ...
  //animate();
    ...
    const brandingTexture = textureLoader.load("./branding_boundry_2.png");//no branding
    ...
    brandingBoard.position.set(0, 0.4, 12); 
...
//ys
 ...
 scan_status_msg.innerHTML = "Scan a rectangualr shaped object";
